...
```
==>  Preparing: SELECT * FROM ( SELECT u.user_id AS user_id, u.user_name AS user_name, u.user_sex AS user_sex, u.user_age AS user_age, j.job_id AS job_id, j.job_name AS job_name FROM `ivan_user` u JOIN `ivan_job` j ON u.user_id = j.user_id ) x WHERE (user_id IN (?,?,?,?,?,?)) LIMIT ?,? 
